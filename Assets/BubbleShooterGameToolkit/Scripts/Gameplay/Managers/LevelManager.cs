...
using com.kshkum.ShootGame.Scripts.Gameplay.FieldObjects;
...
namespace com.kshkum.ShootGame.Scripts.Gameplay.Managers
{
    ...
    public partial class LevelManager : SingletonBehaviour<LevelManager>
    {
        ...
        public void InitGame()
        {
            ...
            {
                Debug.Log("setting up vertical level");
            ...
            Debug.Log($"Initializing the game - {Level.stars.Length} {Level} {instance}" );
        ...
        {
            Debug.Log($"is target down: {TargetManager.instance.IsTargetsDone()} \t game status: {EventManager.GameStatus}");
            if(TargetManager.instance.IsTargetsDone() && EventManager.GameStatus != EStatus.Win)
            {
                Debug.Log("Target achieved, game win");
            ...
            {
                Debug.Log("Target failed, game lost");
        ...
        public void CheckMovesAndTargetsAfterDestroy()
        {
            if (EventManager.GameStatus == EStatus.Play)
            {
                ...
                Debug.Log("game status is playing && Checking moves and targets after destroying");
        ...
        {
            Debug.Log("After destoy any process called by referencing in script");
            StartCoroutine(AfterDestroyProcess(1));
        }

        private IEnumerator AfterDestroyProcess(int countToDestroy)
        {
            Debug.Log("After destoy the process");
            if (countToDestroy > 0)
            {
